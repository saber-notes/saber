...
Future<void> main() async {
  ...
  final allVersions = await Process.run('curl', [
  ...
  final String latestCommit = allVersions['current_release'][channel];

  final String submoduleVersion = (allVersions['releases'] as List).firstWhere(
      (release) =>
          release['hash'] == submoduleCommit &&
          release['channel'] == channel)['version'];
  final String latestVersion = (allVersions['releases'] as List).firstWhere(
      (release) =>
          release['hash'] == latestCommit &&
          release['channel'] == channel)['version'];

  if (submoduleVersion == latestVersion) {
    print('Flutter submodule ($submoduleVersion) is up to date!');
  } else {
    print('Flutter submodule ($submoduleVersion) is out of date: '
