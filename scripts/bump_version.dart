...
Future<void> updateAllFiles() async {
  ...
  if (!failOnChanges) {
    // Populate i18n strings. Skip if we're running a test.
    {
      var start =
          metainfoLines.indexWhere((line) => line.endsWith('<description>')) +
          1;
      ...
      for (final localeCode in [null, ...localesWithFullDescription]) {
        ...
        final openTag = localeCode == null
            ? '<p>'
            : '<p xml:lang="$localeCode">';
